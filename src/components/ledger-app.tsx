...
type Transaction = {
  ...
  transfer_group_id?: string | null; transfer_direction?: "in" | "out" | null; counterparty_account_id?: string | null; counterparty_account_name?: string | null; counterparty_amount_minor?: string | null; counterparty_currency?: string | null;
};
...
type SummaryCurrency = { currency: string; expense_minor: string; refund_minor: string; income_minor: string; transfer_in_minor: string; transfer_out_minor: string; net_expense_minor: string; net_cashflow_minor: string; transaction_count: number; transfer_count: number };
type Summary = { currencies: SummaryCurrency[]; base: (SummaryCurrency & { missing_fx_count: number; coverage: number }) | null; groups: Array<{ label: string; currency: string; net_expense_minor: string; count: number }>; income_groups: Array<{ label: string; currency: string; income_minor: string; count: number }>; exchanges: Array<{ source_currency: string; target_currency: string; source_amount_minor: string; target_amount_minor: string; effective_rate: string; inverse_rate: string; count: number }>; missing_fx_transaction_ids?: string[] };
type AiReport = { report: { observations: Array<{ metric_id: string; summary: string; action: string }>; limitations: string[] } };
type FormState = { kind: Kind; amount: string; currency: string; counterpartyAmount: string; counterpartyCurrency: string; occurredLocal: string; timePrecision: "date" | "minute" | "second"; categoryId: string; paymentMethod: string; accountName: string; counterpartyAccountName: string; channelId: string; merchant: string; note: string; relatedId: string; idempotencyKey: string };
...
function freshForm(timezone = "Asia/Hong_Kong"): FormState {
  return { kind: "expense", amount: "", currency: "HKD", counterpartyAmount: "", counterpartyCurrency: "CNY", occurredLocal: DateTime.now().setZone(timezone).toFormat("yyyy-MM-dd'T'HH:mm"), timePrecision: "minute", categoryId: "", paymentMethod: "", accountName: "", counterpartyAccountName: "", channelId: "", merchant: "", note: "", relatedId: "", idempotencyKey: crypto.randomUUID() };
}
...
const normalizeAccountName = (value: string) => value.normalize("NFKC").trim().toLowerCase();
const accountNameFor = (accounts: Meta[], id?: string | null, fallback?: string | null) => fallback ?? accounts.find((account) => account.id === id)?.name ?? "";
async function resolveAccountId(accounts: Meta[], value: string, currency: string) {
  const name = value.trim(); if (!name) return null;
  const normalized = normalizeAccountName(name);
  const existing = accounts.find((account) => account.id === name || normalizeAccountName(account.name) === normalized);
  if (existing) return existing.id;
  try {
    const created = await api<{ id: string }>("/api/v1/metadata", { method: "POST", body: JSON.stringify({ type: "account", name, currency }) });
    return created.id;
  } catch (caught) {
    // A previous uncertain save may already have created the account even though this tab still has stale metadata.
    if (!(caught instanceof ApiError) || caught.status !== 409) throw caught;
    const refreshed = await api<{ accounts: Meta[] }>("/api/v1/metadata");
    const duplicate = refreshed.accounts.find((account) => normalizeAccountName(account.name) === normalized);
...
function EntryPanel({ data, reload, onMore, copy }: { data: BootstrapUnlocked; reload: () => Promise<void>; onMore: () => void; copy: Transaction | null }) {
  ...
  const [form, setForm] = useState<FormState>(() => copy ? { ...freshForm(timezone), kind: copy.kind, amount: minorToMajor(copy.amount_minor), currency: copy.currency, categoryId: copy.category_id ?? "", paymentMethod: copy.payment_method_id ?? copy.payment_method ?? "", accountName: accountNameFor(data.metadata.accounts, copy.account_id, copy.account_name), channelId: copy.channel_id ?? "", merchant: copy.merchant ?? "", note: copy.note ?? "" } : freshForm(timezone));
  ...
  const sameCurrencyTransfer = form.currency === form.counterpartyCurrency;
  const transferIncomplete = form.kind === "transfer" && (!(sameCurrencyTransfer ? form.amount : form.counterpartyAmount) || !form.accountName.trim() || !form.counterpartyAccountName.trim());
  const set = (key: keyof FormState, value: string) => setForm((current) => ({ ...current, [key]: value }));
  function changeKind(kind: Kind) {
    setForm((current) => ({
      ...current,
      kind,
      categoryId: kind === current.kind || (kind === "expense" || kind === "income") && data.metadata.categories.some((category) => category.id === current.categoryId && category.transaction_kind === kind) ? current.categoryId : "",
      paymentMethod: kind === "expense" ? current.paymentMethod : "",
      channelId: kind === "expense" ? current.channelId : "",
      merchant: kind === "expense" || kind === "income" ? current.merchant : "",
      relatedId: kind === "refund" ? current.relatedId : "",
      counterpartyAmount: kind === "transfer" ? current.counterpartyAmount : "",
      counterpartyCurrency: kind === "transfer" ? (current.counterpartyCurrency === current.currency ? currencies.find((currency) => currency !== current.currency) ?? current.currency : current.counterpartyCurrency) : current.counterpartyCurrency,
      counterpartyAccountName: kind === "transfer" ? current.counterpartyAccountName : "",
    }));
  }
  useEffect(() => {
  ...
  async function save(andContinue: boolean) {
    ...
    try {
      const amountMinor = parseMajorAmount(form.amount, form.currency); const occurred = DateTime.fromISO(form.occurredLocal, { zone: timezone }).toISO({ suppressMilliseconds: true });
      const selectedPayment = form.kind === "expense" ? data.metadata.payment_methods?.find((method) => method.id === form.paymentMethod) : undefined;
      const accountId = await resolveAccountId(data.metadata.accounts, form.accountName, form.currency);
      const counterpartyAccountId = form.kind === "transfer" ? await resolveAccountId(data.metadata.accounts, form.counterpartyAccountName, form.counterpartyCurrency) : null;
      const counterpartyAmountMinor = form.kind === "transfer" ? parseMajorAmount(sameCurrencyTransfer ? form.amount : form.counterpartyAmount, form.counterpartyCurrency) : undefined;
      result = await api<{ deduplicated: boolean }>("/api/v1/transactions", { method: "POST", body: JSON.stringify({ kind: form.kind, amount_minor: amountMinor, currency: form.currency, occurred_at: occurred, occurred_timezone: timezone, time_precision: form.timePrecision, category_id: form.kind === "expense" || form.kind === "income" ? form.categoryId || null : null, payment_method_id: form.kind === "expense" ? selectedPayment?.id ?? null : null, payment_method: form.kind === "expense" ? selectedPayment?.legacy_code ?? (selectedPayment ? null : form.paymentMethod || null) : null, account_id: accountId, counterparty_account_id: counterpartyAccountId, counterparty_amount_minor: counterpartyAmountMinor, counterparty_currency: form.kind === "transfer" ? form.counterpartyCurrency : undefined, channel_id: form.kind === "expense" ? form.channelId || null : null, merchant: form.kind === "expense" || form.kind === "income" ? form.merchant || null : null, note: form.note || null, related_transaction_id: form.kind === "refund" ? form.relatedId || null : null, idempotency_key: form.idempotencyKey, source: aiSuggested ? "ai_confirmed" : "manual" }) });
    } catch (caught) {
  ...
  return <div className="grid"><section className="card entry">
    ...
    <div className="kind-row">{(Object.keys(kindLabels) as Kind[]).map((kind) => <button key={kind} className={`seg ${form.kind === kind ? "active" : ""}`} onClick={() => changeKind(kind)}>{kindLabels[kind]}</button>)}</div>
    {form.kind === "transfer" && <p className="section-label">转出金额</p>}
    <div className="amount-wrap"><span className="currency-label">{form.currency}</span><input className="amount" inputMode="decimal" value={form.amount} onChange={(event) => set("amount", event.target.value.replace(/[^\d.]/g, ""))} placeholder="0.00" aria-label={form.kind === "transfer" ? "转出金额" : "金额"} /></div>
    ...
    {form.kind === "transfer" && <><p className="section-label">实际到账金额{sameCurrencyTransfer ? "（同币种自动一致）" : ""}</p><div className="amount-wrap exchange-amount"><span className="currency-label">{form.counterpartyCurrency}</span><input className="amount" inputMode="decimal" value={sameCurrencyTransfer ? form.amount : form.counterpartyAmount} disabled={sameCurrencyTransfer} onChange={(event) => set("counterpartyAmount", event.target.value.replace(/[^\d.]/g, ""))} placeholder="0.00" aria-label="实际到账金额" /></div><div className="currency-row">{currencies.map((currency) => <button key={currency} className={`currency ${form.counterpartyCurrency === currency ? "active" : ""}`} onClick={() => setForm((current) => ({ ...current, counterpartyCurrency: currency, counterpartyAmount: currency === current.currency ? current.amount : current.counterpartyAmount }))}>{currency}</button>)}</div></>}
    {(form.kind === "expense" || form.kind === "income") && <><p className="section-label">分类</p><div className="chip-row"><button className={`chip ${!form.categoryId ? "active" : ""}`} onClick={() => set("categoryId", "")}>未分类</button>{categories.map((category) => <button key={category.id} className={`chip ${form.categoryId === category.id ? "active" : ""}`} onClick={() => set("categoryId", category.id)}>{category.name}</button>)}</div></>}
    <div className="field-grid">
      ...
      {form.kind === "expense" && <label className="field">付款方式<select className="control" value={form.paymentMethod} onChange={(event) => set("paymentMethod", event.target.value)}><option value="">未指定</option>{(data.metadata.payment_methods?.length ? data.metadata.payment_methods : paymentMethods.map(([value, label]) => ({ id: value, name: label, legacy_code: value }))).map((method) => <option value={method.id} key={method.id}>{method.name}</option>)}</select></label>}
      <label className="field">{form.kind === "transfer" ? "转出账户" : form.kind === "income" ? "收款账户" : form.kind === "refund" ? "退款账户" : "付款账户"}<input className="control" list="entry-account-options" value={form.accountName} maxLength={80} onChange={(event) => set("accountName", event.target.value)} placeholder="选择或输入账户名称" /><datalist id="entry-account-options">{data.metadata.accounts.filter((account) => account.currency === form.currency).map((account) => <option value={account.name} key={account.id} />)}</datalist><small>输入新名称会按 {form.currency} 币种加入账户列表。</small></label>
      {form.kind === "transfer" && <label className="field">转入账户<input className="control" list="entry-counterparty-account-options" value={form.counterpartyAccountName} maxLength={80} onChange={(event) => set("counterpartyAccountName", event.target.value)} placeholder="选择或输入转入账户" /><datalist id="entry-counterparty-account-options">{data.metadata.accounts.filter((account) => account.currency === form.counterpartyCurrency && normalizeAccountName(account.name) !== normalizeAccountName(form.accountName)).map((account) => <option value={account.name} key={account.id} />)}</datalist><small>输入新名称会按 {form.counterpartyCurrency} 币种加入账户列表。</small></label>}
      {form.kind === "expense" && <label className="field">消费渠道<select className="control" value={form.channelId} onChange={(event) => set("channelId", event.target.value)}><option value="">未指定</option>{data.metadata.channels.map((channel) => <option value={channel.id} key={channel.id}>{channel.name}</option>)}</select></label>}
      {(form.kind === "expense" || form.kind === "income") && <label className="field">{form.kind === "income" ? "收入来源" : "商家"}<input className="control" value={form.merchant} maxLength={160} onChange={(event) => set("merchant", event.target.value)} placeholder={form.kind === "income" ? "例如：雇主、客户" : "实际商家"} /></label>}
    ...
    <div className="actions"><button className="btn secondary" disabled={busy || !form.amount || transferIncomplete || (form.kind === "refund" && !form.relatedId)} onClick={() => save(true)}>{continueAfter && busy ? "保存中…" : "保存并继续"}</button><button className="btn primary" disabled={busy || !form.amount || transferIncomplete || (form.kind === "refund" && !form.relatedId)} onClick={() => save(false)}><Save size={17} />{!continueAfter && busy ? "正在保存…" : phase === "uncertain" ? "重试保存" : "保存"}</button></div>
...
function TransactionRow({ tx, timezone, actions }: { tx: Transaction; timezone: string; actions?: React.ReactNode }) {
  const sign = tx.kind === "expense" ? "−" : tx.kind === "transfer" ? tx.transfer_direction === "out" ? "−" : tx.transfer_direction === "in" ? "+" : "↔" : "+";
  const exchange = tx.kind === "transfer" && tx.counterparty_currency && tx.counterparty_currency !== tx.currency;
  const transferDirection = tx.transfer_direction === "in" ? "转入" : tx.transfer_direction === "out" ? "转出" : "";
  const title = tx.kind === "transfer" ? `${exchange ? "换汇" : "账户转账"}${transferDirection}` : tx.merchant || tx.category_name || kindLabels[tx.kind];
  const details = [fmtDate(tx.occurred_at, timezone)];
  if (tx.kind === "expense") details.push(tx.payment_method_name ?? (tx.payment_method ? paymentLabels[tx.payment_method] ?? tx.payment_method : "付款方式未指定"));
  if (tx.account_name) details.push(tx.account_name);
  if (tx.kind === "transfer" && tx.counterparty_amount_minor && tx.counterparty_currency) details.push(`${tx.transfer_direction === "in" ? "对应转出" : "实际到账"} ${formatMinor(tx.counterparty_amount_minor, tx.counterparty_currency)}`);
  if (tx.source === "mcp") details.push("Agent");
  return <div className="transaction"><div className="tx-icon">{iconFor(tx)}</div><div className="tx-main"><div className="tx-name">{title}</div><div className="tx-meta">{details.join(" · ")}</div>{actions}</div><div className={`tx-amount ${tx.kind}`}>{sign}{formatMinor(tx.amount_minor, tx.currency)}</div></div>;
}
...
function ListPanel({ data, reload, copy }: { data: BootstrapUnlocked; reload: () => Promise<void>; copy: (tx: Transaction) => void }) {
  ...
  return <section className="panel"><div className="card panel-card"><div className="side-head"><h1 className="panel-title">流水</h1><button className="link-btn" onClick={() => setFilters({ kind: "", categoryId: "", paymentMethod: "", accountId: "", channelId: "", dateFrom: "", dateTo: "", search: "" })}>清除筛选</button></div>
    ...
    {error && <div className="notice error">{error}</div>}{items.length ? items.map((tx) => <TransactionRow key={tx.id} tx={tx} timezone={timezone} actions={<div className="row-actions">{!tx.transfer_group_id && <button onClick={() => copy(tx)}><Copy size={13} />复制</button>}{!tx.transfer_group_id && <button onClick={() => setEditing(tx)}><BookOpen size={13} />编辑</button>}<button onClick={() => remove(tx)}><Trash2 size={13} />撤销</button></div>} />) : !loading && <div className="empty">当前筛选下没有流水</div>}{loading && <div className="spinner list-spinner" />}{nextCursor && !loading && <button className="btn ghost full-button" onClick={loadMore}>加载更多</button>}
...
function EditSheet({ tx, data, close, saved }: { tx: Transaction; data: BootstrapUnlocked; close: () => void; saved: () => Promise<void> }) {
  const timezone = data.profile.timezone;
  const [form, setForm] = useState({ amount: minorToMajor(tx.amount_minor), currency: tx.currency, occurredLocal: DateTime.fromISO(tx.occurred_at).setZone(timezone).toFormat("yyyy-MM-dd'T'HH:mm"), timePrecision: tx.time_precision, categoryId: tx.category_id ?? "", paymentMethod: tx.payment_method_id ?? tx.payment_method ?? "", accountName: accountNameFor(data.metadata.accounts, tx.account_id, tx.account_name), channelId: tx.channel_id ?? "", merchant: tx.merchant ?? "", note: tx.note ?? "" });
  ...
  async function submit() { setBusy(true); try { const amountMinor = parseMajorAmount(form.amount, form.currency); const selectedPayment = tx.kind === "expense" ? data.metadata.payment_methods?.find((method) => method.id === form.paymentMethod) : undefined; const accountId = await resolveAccountId(data.metadata.accounts, form.accountName, form.currency); await api(`/api/v1/transactions/${tx.id}`, { method: "PATCH", body: JSON.stringify({ version: tx.version, amount_minor: amountMinor, currency: form.currency, occurred_at: DateTime.fromISO(form.occurredLocal, { zone: timezone }).toISO({ suppressMilliseconds: true }), occurred_timezone: timezone, time_precision: form.timePrecision, category_id: tx.kind === "expense" || tx.kind === "income" ? form.categoryId || null : null, payment_method_id: tx.kind === "expense" ? selectedPayment?.id ?? null : null, payment_method: tx.kind === "expense" ? selectedPayment?.legacy_code ?? (selectedPayment ? null : form.paymentMethod || null) : null, account_id: accountId, channel_id: tx.kind === "expense" ? form.channelId || null : null, merchant: tx.kind === "expense" || tx.kind === "income" ? form.merchant || null : null, note: form.note || null }) }); await saved(); } catch (caught) { alert((caught as Error).message); setBusy(false); } }
  return <div className="overlay" onClick={close}><div className="sheet card" onClick={(event) => event.stopPropagation()}><button className="close" onClick={close} aria-label="关闭"><X /></button><p className="eyebrow">EDIT</p><h2 className="panel-title">修改账目</h2><div className="field-grid edit-grid">
    <label className="field">金额<input className="control" inputMode="decimal" value={form.amount} onChange={(event) => set("amount", event.target.value.replace(/[^\d.]/g, ""))} /></label>
    <label className="field">币种<select className="control" value={form.currency} onChange={(event) => set("currency", event.target.value)}>{currencies.map((currency) => <option key={currency}>{currency}</option>)}</select></label>
    <label className="field">发生时间<input type="datetime-local" className="control" value={form.occurredLocal} onChange={(event) => set("occurredLocal", event.target.value)} /></label>
    {(tx.kind === "expense" || tx.kind === "income") && <label className="field">分类<select className="control" value={form.categoryId} onChange={(event) => set("categoryId", event.target.value)}><option value="">未分类</option>{categories.map((category) => <option value={category.id} key={category.id}>{category.name}</option>)}</select></label>}
    {tx.kind === "expense" && <label className="field">付款方式<select className="control" value={form.paymentMethod} onChange={(event) => set("paymentMethod", event.target.value)}><option value="">未指定</option>{(data.metadata.payment_methods?.length ? data.metadata.payment_methods : paymentMethods.map(([value, label]) => ({ id: value, name: label, legacy_code: value }))).map((method) => <option value={method.id} key={method.id}>{method.name}</option>)}</select></label>}
    <label className="field">{tx.kind === "income" ? "收款账户" : tx.kind === "refund" ? "退款账户" : tx.kind === "transfer" ? "转账账户" : "付款账户"}<input className="control" list="edit-account-options" value={form.accountName} maxLength={80} onChange={(event) => set("accountName", event.target.value)} placeholder="选择或输入账户名称" /><datalist id="edit-account-options">{data.metadata.accounts.filter((account) => account.currency === form.currency).map((account) => <option value={account.name} key={account.id} />)}</datalist><small>输入新名称会按 {form.currency} 币种加入账户列表。</small></label>
    {tx.kind === "expense" && <label className="field">消费渠道<select className="control" value={form.channelId} onChange={(event) => set("channelId", event.target.value)}><option value="">未指定</option>{data.metadata.channels.map((channel) => <option value={channel.id} key={channel.id}>{channel.name}</option>)}</select></label>}
    {(tx.kind === "expense" || tx.kind === "income") && <label className="field">{tx.kind === "income" ? "收入来源" : "商家"}<input className="control" value={form.merchant} onChange={(event) => set("merchant", event.target.value)} /></label>}
    <label className="field full">备注<textarea className="control" rows={3} value={form.note} onChange={(event) => set("note", event.target.value)} /></label>
  </div><button className="btn primary full-button" onClick={submit} disabled={busy || !form.amount}>{busy ? "保存中…" : "保存修改"}</button></div></div>;
}
...
function StatsPanel({ data }: { data: BootstrapUnlocked }) {
  ...
  const spendingMax = Math.max(1, ...summary.groups.map((group) => Math.abs(Number(group.net_expense_minor))));
  const incomeMax = Math.max(1, ...summary.income_groups.map((group) => Number(group.income_minor)));
  const shown = currencyMode === "base" && summary.base ? [summary.base] : summary.currencies;
  const metricValue = (metricId: string) => {
    const group = /^group_(\d+)$/.exec(metricId);
    if (group) { const item = summary.groups[Number(group[1])]; return item ? `${item.label} 消费净额 ${formatMinor(item.net_expense_minor, item.currency)}` : "指标不可用"; }
    const incomeGroup = /^income_group_(\d+)$/.exec(metricId);
    if (incomeGroup) { const item = summary.income_groups[Number(incomeGroup[1])]; return item ? `${item.label} 收入 ${formatMinor(item.income_minor, item.currency)}` : "指标不可用"; }
    const exchange = /^exchange_(\d+)_rate$/.exec(metricId);
    if (exchange) { const item = summary.exchanges[Number(exchange[1])]; return item ? `实际成交汇率 1 ${item.source_currency} = ${item.effective_rate} ${item.target_currency}` : "指标不可用"; }
    const match = /^currency_(\d+)_(expense|income|transfer_in|transfer_out|balance)$/.exec(metricId); if (!match) return "指标不可用"; const currency = summary.currencies[Number(match[1])]; if (!currency) return "指标不可用"; const values = { expense: currency.expense_minor, income: currency.income_minor, transfer_in: currency.transfer_in_minor, transfer_out: currency.transfer_out_minor, balance: currency.net_cashflow_minor }; const labels = { expense: "支出", income: "收入", transfer_in: "转账转入", transfer_out: "转账转出", balance: "现金流净额" }; const metric = match[2] as keyof typeof values; return `${currency.currency} ${labels[metric]} ${formatMinor(values[metric], currency.currency)}`;
  };
  ...
  return <section className="panel">
    <div className="card panel-card">
      <RangeSwitch mode={rangeMode} setMode={setRangeMode} month={month} setMonth={setMonth} currentMonth={currentMonth} from={customFrom} setFrom={setCustomFrom} to={customTo} setTo={setCustomTo} />
      <div className="filter-grid"><select className="control" value={currencyMode} onChange={(event) => setCurrencyMode(event.target.value as "original" | "base")}><option value="original">原币视图</option><option value="base">统一换算</option></select><select className="control" value={displayCurrency} disabled={currencyMode === "original"} onChange={(event) => setDisplayCurrency(event.target.value)}>{currencies.map((currency) => <option key={currency}>{currency}</option>)}</select><select className="control" value={groupBy} onChange={(event) => setGroupBy(event.target.value)}><option value="category">按分类</option><option value="payment_method">按支付方式</option><option value="account">按账户</option><option value="channel">按渠道</option><option value="merchant">按商家</option></select>{groupBy === "category" && <select className="control" value={categoryLevel} onChange={(event) => setCategoryLevel(event.target.value as "top" | "leaf")}><option value="top">一级分类汇总</option><option value="leaf">明细分类</option></select>}</div>
      {shown.length ? shown.map((currency) => {
        const cashflow = BigInt(currency.net_cashflow_minor);
        const absoluteCashflow = cashflow < 0n ? -cashflow : cashflow;
        const cashflowLabel = cashflow > 0n ? "本期净流入" : cashflow < 0n ? "本期净流出" : "本期收支平衡";
        const showTransfers = currencyMode === "original" && (BigInt(currency.transfer_in_minor) !== 0n || BigInt(currency.transfer_out_minor) !== 0n);
        return <div key={currency.currency} className="summary-block"><div className="stat-label">{currency.currency} {cashflowLabel}</div><div className="summary-total">{formatMinor(absoluteCashflow.toString(), currency.currency)}</div><div className="summary-grid"><div className="stat"><div className="stat-label">支出</div><div className="stat-value">{formatMinor(currency.expense_minor, currency.currency)}</div></div><div className="stat"><div className="stat-label">退款</div><div className="stat-value">{formatMinor(currency.refund_minor, currency.currency)}</div></div><div className="stat"><div className="stat-label">收入</div><div className="stat-value">{formatMinor(currency.income_minor, currency.currency)}</div></div>{showTransfers && <><div className="stat"><div className="stat-label">转账转入</div><div className="stat-value">{formatMinor(currency.transfer_in_minor, currency.currency)}</div></div><div className="stat"><div className="stat-label">转账转出</div><div className="stat-value">{formatMinor(currency.transfer_out_minor, currency.currency)}</div></div></>}</div></div>;
      }) : <div className="empty">本月还没有账目</div>}
      {currencyMode === "original" && summary.exchanges.length > 0 && <div className="notice pending">原币净现金流包含换汇及账户转账；支出、退款和收入仍只统计经营收支。</div>}
      {currencyMode === "base" && summary.base && summary.base.missing_fx_count > 0 && <div className="notice pending">有 {summary.base.missing_fx_count} 笔外币账目缺少可用历史汇率；覆盖率 {(summary.base.coverage * 100).toFixed(0)}%，这些交易未计入换算合计。</div>}
    </div>
    {summary.exchanges.length > 0 && <div className="card panel-card"><h2 className="panel-title">本期实际换汇</h2>{summary.exchanges.map((exchange) => <div className="summary-block" key={`${exchange.source_currency}-${exchange.target_currency}`}><div className="stat-label">{exchange.source_currency} → {exchange.target_currency} · {exchange.count} 笔</div><div className="summary-total">1 {exchange.source_currency} = {exchange.effective_rate} {exchange.target_currency}</div><div className="muted-copy">实际换出 {formatMinor(exchange.source_amount_minor, exchange.source_currency)}，实际换入 {formatMinor(exchange.target_amount_minor, exchange.target_currency)}；反向汇率 1 {exchange.target_currency} = {exchange.inverse_rate} {exchange.source_currency}</div></div>)}</div>}
    <div className="card panel-card"><h2 className="panel-title">{groupBy === "category" ? "支出分类" : "支出分组"}</h2>{summary.groups.length ? summary.groups.map((group) => <div className="bar-row" key={`${group.label}-${group.currency}`}><div className="bar-label"><span>{group.label}</span><strong>{formatMinor(group.net_expense_minor, group.currency)}</strong></div><div className="bar"><div className="bar-fill" style={{ width: `${Math.max(3, Math.abs(Number(group.net_expense_minor)) / spendingMax * 100)}%` }} /></div></div>) : <div className="empty">暂无可汇总支出</div>}</div>
    <div className="card panel-card"><h2 className="panel-title">{groupBy === "category" ? "收入分类" : "收入分组"}</h2>{summary.income_groups.length ? summary.income_groups.map((group) => <div className="bar-row" key={`${group.label}-${group.currency}`}><div className="bar-label"><span>{group.label}</span><strong>{formatMinor(group.income_minor, group.currency)}</strong></div><div className="bar"><div className="bar-fill" style={{ width: `${Math.max(3, Number(group.income_minor) / incomeMax * 100)}%` }} /></div></div>) : <div className="empty">暂无可汇总收入</div>}</div>
    <div className="card panel-card"><h2 className="panel-title">AI 收支观察</h2>{data.ai_configured ? <><p className="muted-copy">模型只解释代码生成的统计快照，不会收到逐笔商户或备注。</p><button className="btn secondary" onClick={createAiReport} disabled={reportBusy || !summary.currencies.length}><Sparkles size={16} />{reportBusy ? "分析中…" : "生成本月观察"}</button>{reportError && <div className="notice error">{reportError}</div>}{report?.report.observations.map((item, index) => <div className="ai-observation" key={`${item.metric_id}-${index}`}><strong>{metricValue(item.metric_id)}</strong><p>{item.summary}</p><small>{item.action}</small></div>)}{report?.report.limitations.map((item, index) => <div className="notice pending" key={index}>{item}</div>)}</> : <div className="empty">AI 默认关闭；手工记账与确定性统计不受影响。</div>}</div>
  </section>;
}
