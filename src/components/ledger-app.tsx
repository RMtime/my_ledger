...
type Summary = { currencies: Array<{ currency: string; expense_minor: string; refund_minor: string; income_minor: string; net_expense_minor: string; net_cashflow_minor: string; transaction_count: number }>; base: { currency: string; expense_minor: string; refund_minor: string; income_minor: string; net_expense_minor: string; net_cashflow_minor: string; missing_fx_count: number; coverage: number } | null; groups: Array<{ label: string; currency: string; net_expense_minor: string; count: number }>; income_groups: Array<{ label: string; currency: string; income_minor: string; count: number }>; missing_fx_transaction_ids?: string[] };
...
function StatsPanel({ data }: { data: BootstrapUnlocked }) {
  ...
  const metricValue = (metricId: string) => {
    ...
    const match = /^currency_(\d+)_(expense|income|balance)$/.exec(metricId); if (!match) return "指标不可用"; const currency = summary.currencies[Number(match[1])]; if (!currency) return "指标不可用"; const values = { expense: currency.expense_minor, income: currency.income_minor, balance: currency.net_cashflow_minor }; const labels = { expense: "支出", income: "收入", balance: "收支结余" }; const metric = match[2] as keyof typeof values; return `${currency.currency} ${labels[metric]} ${formatMinor(values[metric], currency.currency)}`;
  };
