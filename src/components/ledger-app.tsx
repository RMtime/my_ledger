...
type FormState = { kind: Kind; amount: string; currency: string; occurredLocal: string; timePrecision: "date" | "minute" | "second"; categoryId: string; paymentMethod: string; accountName: string; counterpartyAccountName: string; channelId: string; merchant: string; note: string; relatedId: string; idempotencyKey: string };
...
function freshForm(timezone = "Asia/Hong_Kong"): FormState {
  return { kind: "expense", amount: "", currency: "HKD", occurredLocal: DateTime.now().setZone(timezone).toFormat("yyyy-MM-dd'T'HH:mm"), timePrecision: "minute", categoryId: "", paymentMethod: "", accountName: "", counterpartyAccountName: "", channelId: "", merchant: "", note: "", relatedId: "", idempotencyKey: crypto.randomUUID() };
}
...
function EntryPanel({ data, reload, onMore, copy }: { data: BootstrapUnlocked; reload: () => Promise<void>; onMore: () => void; copy: Transaction | null }) {
  ...
  return <div className="grid"><section className="card entry">
    ...
    <div className="field-grid">
      ...
      <label className="field">{form.kind === "transfer" ? "转出账户" : "付款账户"}<input className="control" list="entry-account-options" value={form.accountName} maxLength={80} onChange={(event) => set("accountName", event.target.value)} placeholder="选择或输入账户名称" /><datalist id="entry-account-options">{data.metadata.accounts.map((account) => <option value={account.name} key={account.id} />)}</datalist><small>输入新名称会在保存时加入账户列表。</small></label>
      {form.kind === "transfer" && <label className="field">转入账户<input className="control" list="entry-counterparty-account-options" value={form.counterpartyAccountName} maxLength={80} onChange={(event) => set("counterpartyAccountName", event.target.value)} placeholder="选择或输入对手账户" /><datalist id="entry-counterparty-account-options">{data.metadata.accounts.filter((account) => normalizeAccountName(account.name) !== normalizeAccountName(form.accountName)).map((account) => <option value={account.name} key={account.id} />)}</datalist></label>}
...
function TransactionRow({ tx, timezone, actions }: { tx: Transaction; timezone: string; actions?: React.ReactNode }) {
  ...
  return <div className="transaction"><div className="tx-icon">{iconFor(tx)}</div><div className="tx-main"><div className="tx-name">{tx.merchant || tx.category_name || kindLabels[tx.kind]}</div><div className="tx-meta">{fmtDate(tx.occurred_at, timezone)} · {tx.payment_method_name ?? (tx.payment_method ? paymentLabels[tx.payment_method] ?? tx.payment_method : "未指定")}{tx.account_name ? ` · ${tx.account_name}` : ""}{tx.source === "mcp" ? " · Agent" : ""}</div>{actions}</div><div className={`tx-amount ${tx.kind}`}>{sign}{formatMinor(tx.amount_minor, tx.currency)}</div></div>;
}
...
function EditSheet({ tx, data, close, saved }: { tx: Transaction; data: BootstrapUnlocked; close: () => void; saved: () => Promise<void> }) {
  ...
  async function submit() { setBusy(true); try { const amountMinor = parseMajorAmount(form.amount, form.currency); const selectedPayment = data.metadata.payment_methods?.find((method) => method.id === form.paymentMethod); const accountId = await resolveAccountId(data.metadata.accounts, form.accountName, form.currency); await api(`/api/v1/transactions/${tx.id}`, { method: "PATCH", body: JSON.stringify({ version: tx.version, amount_minor: amountMinor, currency: form.currency, occurred_at: DateTime.fromISO(form.occurredLocal, { zone: timezone }).toISO({ suppressMilliseconds: true }), occurred_timezone: timezone, time_precision: form.timePrecision, category_id: form.categoryId || null, payment_method_id: selectedPayment?.id ?? null, payment_method: selectedPayment?.legacy_code ?? (selectedPayment ? null : form.paymentMethod || null), account_id: accountId, channel_id: form.channelId || null, merchant: form.merchant || null, note: form.note || null }) }); await saved(); } catch (caught) { alert((caught as Error).message); setBusy(false); } }
  return <div className="overlay" onClick={close}><div className="sheet card" onClick={(event) => event.stopPropagation()}><button className="close" onClick={close} aria-label="关闭"><X /></button><p className="eyebrow">EDIT</p><h2 className="panel-title">修改账目</h2><div className="field-grid edit-grid"><label className="field">金额<input className="control" inputMode="decimal" value={form.amount} onChange={(event) => set("amount", event.target.value.replace(/[^\d.]/g, ""))} /></label><label className="field">币种<select className="control" value={form.currency} onChange={(event) => set("currency", event.target.value)}>{currencies.map((currency) => <option key={currency}>{currency}</option>)}</select></label><label className="field">发生时间<input type="datetime-local" className="control" value={form.occurredLocal} onChange={(event) => set("occurredLocal", event.target.value)} /></label><label className="field">分类<select className="control" value={form.categoryId} onChange={(event) => set("categoryId", event.target.value)}><option value="">未分类</option>{categories.map((category) => <option value={category.id} key={category.id}>{category.name}</option>)}</select></label><label className="field">付款方式<select className="control" value={form.paymentMethod} onChange={(event) => set("paymentMethod", event.target.value)}><option value="">未指定</option>{(data.metadata.payment_methods?.length ? data.metadata.payment_methods : paymentMethods.map(([value, label]) => ({ id: value, name: label, legacy_code: value }))).map((method) => <option value={method.id} key={method.id}>{method.name}</option>)}</select></label><label className="field">账户<input className="control" list="edit-account-options" value={form.accountName} maxLength={80} onChange={(event) => set("accountName", event.target.value)} placeholder="选择或输入账户名称" /><datalist id="edit-account-options">{data.metadata.accounts.map((account) => <option value={account.name} key={account.id} />)}</datalist><small>输入新名称会在保存时加入账户列表。</small></label><label className="field">渠道<select className="control" value={form.channelId} onChange={(event) => set("channelId", event.target.value)}><option value="">未指定</option>{data.metadata.channels.map((channel) => <option value={channel.id} key={channel.id}>{channel.name}</option>)}</select></label><label className="field">商家<input className="control" value={form.merchant} onChange={(event) => set("merchant", event.target.value)} /></label><label className="field full">备注<textarea className="control" rows={3} value={form.note} onChange={(event) => set("note", event.target.value)} /></label></div><button className="btn primary full-button" onClick={submit} disabled={busy || !form.amount}>{busy ? "保存中…" : "保存修改"}</button></div></div>;
}
