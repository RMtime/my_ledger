...
function SettingsPanel({ data, reload }: { data: Bootstrap; reload: () => Promise<void> }) {
  const [agents, setAgents] = useState<Array<Record<string, string>>>([]); const [token, setToken] = useState(""); const [name, setName] = useState(""); const [metaName, setMetaName] = useState(""); const [metaType, setMetaType] = useState("category");
  useEffect(() => { api<Array<Record<string, string>>>("/api/v1/agents").then(setAgents).catch(() => {}); }, []);
  async function issue() { const result = await api<{ token: string }>("/api/v1/agents", { method: "POST", body: JSON.stringify({ agent_name: name, permissions: ["metadata:read", "transactions:create"] }) }); setToken(result.token); setName(""); setAgents(await api("/api/v1/agents")); }
