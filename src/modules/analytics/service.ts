...
export function getSummary(actor: ActorContext, input: { start: string; end: string; group_by?: keyof typeof allowedGroups; currency_mode?: "original" | "base" }) {
  ...
  if (!input.start || !input.end || Date.parse(input.start) >= Date.parse(input.end)) throw new AppError("VALIDATION_ERROR", "统计区间不正确", 422);
  const rows = sqlite.prepare(`SELECT t.currency,
    ...
    GROUP BY t.currency ORDER BY t.currency`).all(actor.ownerId, input.start, input.end) as Array<Record<string, bigint | string | number>>;
  ...
  const fx = sqlite.prepare(`SELECT p.base_currency,
    ...
    COUNT(*) total_count
    FROM transactions t JOIN profiles p ON p.id=t.owner_id LEFT JOIN transaction_fx f ON f.transaction_id=t.id
    WHERE t.owner_id=? AND t.occurred_at>=? AND t.occurred_at<? AND t.deleted_at IS NULL
      AND t.kind IN ('expense','income','refund') GROUP BY p.base_currency`).get(actor.ownerId, input.start, input.end) as Record<string, bigint | string | number> | undefined;
  const groupExpression = allowedGroups[input.group_by ?? "category"];
  const groups = sqlite.prepare(`SELECT ${groupExpression} label,t.currency,
    ...
    GROUP BY label,t.currency ORDER BY ABS(net_expense_minor) DESC LIMIT 30`).all(actor.ownerId, input.start, input.end) as Array<Record<string, bigint | string | number>>;
  return {
    period: { start: input.start, end: input.end, semantics: "[start,end)", timezone: "由调用者将用户时区边界转换为 UTC" },
    currencies,
