...
describe("AI quota reservation", () => {
  ...
  it.each([
  ...
  ])("uses DeepSeek's default thinking and retries a %s structured response", async (label, firstChoice) => {
    ...
    expect(calls).toBe(2);
    expect(requests[0]).toMatchObject({ max_tokens: 8192, response_format: { type: "json_object" } });
    expect(requests[1]).toMatchObject({ max_tokens: 16384, response_format: { type: "json_object" } });
    expect(requests[0]).not.toHaveProperty("thinking"); expect(requests[0]).not.toHaveProperty("reasoning_effort");
  ...
  it("separates MiniMax reasoning, strips fallback think tags, and retries malformed output", async () => {
    updateAiPreferences(actor, { enabled: true, provider: "minimax", consent_version: "test-v1" });
    const requests: Array<Record<string, unknown>> = []; let calls = 0;
    vi.stubGlobal("fetch", vi.fn(async (_input: RequestInfo | URL, init?: RequestInit) => {
      calls += 1; requests.push(JSON.parse(String(init?.body)) as Record<string, unknown>);
      const content = calls === 1
        ? "<think>private chain without a closing tag"
        : `<think>private chain</think>\n\`\`\`json\n${JSON.stringify(candidate)}\n\`\`\``;
      return new Response(JSON.stringify({ choices: [{ finish_reason: "stop", message: { content, reasoning_details: [{ type: "reasoning.text", text: "private chain" }] } }], usage: { prompt_tokens: 10, completion_tokens: 100 } }), { status: 200, headers: { "content-type": "application/json" } });
    }));
    try {
      await expect(extractCandidate(actor, "synthetic minimax retry", "2026-09-06T12:00:00+08:00", "Asia/Hong_Kong")).resolves.toMatchObject({ candidate, model: "synthetic-minimax-model", cached: false });
      expect(calls).toBe(2);
      expect(requests[0]).toMatchObject({ reasoning_split: true, max_completion_tokens: 8192 });
      expect(requests[1]).toMatchObject({ reasoning_split: true, max_completion_tokens: 16384 });
      expect(requests[0]).not.toHaveProperty("thinking"); expect(requests[0]).not.toHaveProperty("reasoning_effort");
      const invocation = sqlite.prepare("SELECT id FROM ai_invocations WHERE owner_id=? ORDER BY rowid DESC LIMIT 1").get(owner) as { id: string };
